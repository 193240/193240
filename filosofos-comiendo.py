...
class filosofo(threading.Thread):
    ...
    def __init__(self):
        ...
        filosofo.estado.append('Esperando') #EL FILOSOFO ENTRA A LA MESA EN ESTADO ESPERANDO
        filosofo.tenedores.append(threading.Semaphore(0)) #AGREGA EL SEMAFORO DE SU TENEDOR( TENEDOR A LA IZQUIERDA)
        print("Filosofo {0} - Esperando".format(self.id))

    def __del__(self):
        print("Filosofo {0} - ya termino".format(self.id))  #NECESARIO PARA SABER CUANDO TERMINA EL THREAD

    def esperar(self):
        time.sleep(random.randint(0,5)) #CADA FILOSOFO SE TOMA DISTINTO TIEMPO PARA ESPERAR, ALEATORIO
    ...
    def verificar(self,i):
        ...
        if filosofo.estado[i] == 'HAMBRIENTO' and filosofo.estado[self.izquierda(i)] != 'COMIENDO' and filosofo.estado[self.derecha(i)] != 'COMIENDO':
            ...
            #print(filosofo.tenedores[i])
    def tomar(self):
        filosofo.semaforo.acquire() #SEÑALA QUE TOMARA LOS TENEDORES (EXCLUSION MUTUA) bloquea
    ...
    def soltar(self):
        ...
        filosofo.estado[self.id] = 'Termino'
        #comer= False
    ...
    def comer(self):
        ...
        print("FILOSOFO {0} TERMINO DE COMER".format(self.id))

    def run(self):
            self.esperar() #EL FILOSOFO PIENSA
